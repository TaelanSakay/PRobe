    ...
    def trace(node: ast.AST, depth: int, path: List[str]) -> ProvenanceTrace:
        ...
        if isinstance(node, ast.Name):
            ...
            if node.id in scope_map:
                ...
                # Pass resolved origin through unchanged, including HARDCODED and
                # LOCAL_COMPUTED. Do not upgrade UNKNOWN or bump literals to
                # LOCAL_COMPUTED; origin reflects ultimate source uncertainty.
                return resolved
        ...
        if isinstance(node, ast.Attribute):
            ...
            value_trace = trace(node.value, depth + 1, path + ["attribute access"])
            return ProvenanceTrace(
        ...
        if isinstance(node, ast.Subscript):
            ...
            value_trace = trace(node.value, depth + 1, path + ["subscript"])
            return ProvenanceTrace(
                origin=value_trace.origin,
                confidence=value_trace.confidence,
                path=value_trace.path,
        ...
        if isinstance(node, ast.Call):
            ...
            arg_traces = [trace(arg, depth + 1, path) for arg in node.args]
            arg_traces.extend(
                trace(keyword.value, depth + 1, path) for keyword in node.keywords
            )
            if arg_traces:
        ...
        if isinstance(node, ast.BinOp):
            left_trace = trace(node.left, depth + 1, path)
            right_trace = trace(node.right, depth + 1, path)
        ...
        if isinstance(node, ast.JoinedStr):
            ...
            for value in node.values:
                if isinstance(value, ast.FormattedValue):
                    operand_traces.append(trace(value.value, depth + 1, path))
                else:
                    operand_traces.append(trace(value, depth + 1, path))
            return _merge_traces(operand_traces, "f-string")

        if isinstance(node, ast.FormattedValue):
            return trace(node.value, depth + 1, path + ["formatted value"])
